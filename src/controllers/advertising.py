...
from models.advertising import CreateAdvertisingResponse, CreateAdvertising, FilterParams, UpdateAdvertising
...
def create_advertising(request: Request, payload: CreateAdvertising, response: Response, config: AppConfig = Depends(AppConfig)):    
...
def read_advertising(filter_query: Annotated[FilterParams, Query()], request: Request, response: Response, config: AppConfig = Depends(AppConfig)):
    repo = AdvertisingRepo(db=request.app.state.db)
...
def update_advertising(request: Request, payload: UpdateAdvertising, response: Response, config: AppConfig = Depends(AppConfig)):
    repo = AdvertisingRepo(db=request.app.state.db)
    AdvertisingDomain(repo=repo, config=config).update(payload)
    return Response(status_code=200)
    
@router.delete('/advertising', response_model=None)
def delete_advertising(request: Request, response: Response, id: str, config: AppConfig = Depends(AppConfig)):
    repo = AdvertisingRepo(db=request.app.state.db)
